...
public class ChipsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChipsAdapter(Context context, ChipsInput chipsInput, RecyclerView recycler) {
        ...
        mRecycler = recycler;
        mHintFullName = mChipsInput.getHint();
        mEditText = mChipsInput.getEditText();
        mEditText.setImeOptions(EditorInfo.IME_ACTION_DONE | EditorInfo.IME_FLAG_NO_EXTRACT_UI);
        initEditText();
    }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        if (position == mChipList.size()) {
            if (mChipList.size() == 0)
                mEditText.setHint(mHintFullName);
    ...
    public void removeChipById(Object id) {
        for (Iterator<ChipInterface> iter = mChipList.listIterator(); iter.hasNext(); ) {
            ...
            if (chip.getUserIndex() != null && chip.getUserIndex().equals(id)) {
                // remove chip
                iter.remove();
                // notify listener
                mChipsInput.onChipRemoved(chip, mChipList.size());
    ...
    public void removeChipByFullName(String FullName) {
        for (Iterator<ChipInterface> iter = mChipList.listIterator(); iter.hasNext(); ) {
            ChipInterface chip = iter.next();
            if (chip.getFullName().equals(FullName)) {
                // remove chip
                iter.remove();
                // notify listener
                mChipsInput.onChipRemoved(chip, mChipList.size());
    ...
    public void removeChipByInfo(String UserName) {
        for (Iterator<ChipInterface> iter = mChipList.listIterator(); iter.hasNext(); ) {
            ChipInterface chip = iter.next();
            if (chip.getUserName() != null && chip.getUserName().equals(UserName)) {
                // remove chip
                iter.remove();
                // notify listener
                mChipsInput.onChipRemoved(chip, mChipList.size());
    ...
    private boolean listContains(List<ChipInterface> contactList, ChipInterface chip) {
        ...
        if (mChipsInput.getChipValidator() != null) {
            ...
            for (ChipInterface item : contactList) {
                if (chip.getUserIndex() != null && chip.getUserIndex().equals(item.getUserIndex()))
                    return true;
                if (chip.getUserIndex().equals(item.getUserIndex()))
                    return true;
            }
        }
